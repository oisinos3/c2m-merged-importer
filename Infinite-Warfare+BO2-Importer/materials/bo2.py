...
def createNodeGroup(codMaterial, images_path):
    ...
    for texture in codMaterial.Textures:
        ...
        blender_image = bpy.data.images.load(filepath=imagePath)
        ...
        if texture.TexType.startswith("color"):
        ...
        # normalMap
            ...
            loc_y -= 300
        # specularMap
        elif texture.TexType.startswith("spec"):
            ...
            if (texture.TexType == "specGloss"):
                ...
                links.new(sepRGB_node.outputs[1], group_outputs.inputs['Specular'])
            else:
...
def createMaterial(matList, map_materials):
    ...
    if ("glass" in materialName or materialName.startswith("decal_") or materialName.endswith("_blend")) and len(matList) == 1 and "fiberglass" not in materialName:
        material.blend_method = "BLEND"
        principled_node.inputs["Transmission"].default_value = 1.0
    material_output.location = (1000, -100)
    location_x = -500
    location_y = 0
    # Create mix material
    print(len(matList))
    if len(matList)>1:
        ...
        for index, mat in enumerate(matList):
            ...
            if index != 0:
                ...
                if codMaterial.TechSet == "multiply":
                    blend_node.node_tree = bpy.data.node_groups['MultiplyTextures']
                else:
...
def createMixMultiplyGroup():
    # Create a group
    node_group = bpy.data.node_groups.new("MultiplyTextures", 'ShaderNodeTree')
    # Create input & output nodes
    group_outputs = node_group.nodes.new('NodeGroupOutput')
    group_outputs.location = (500,0)
    group_inputs = node_group.nodes.new('NodeGroupInput')
    ...
    node_group.links.new(group_inputs.outputs['Alpha'], mix_color.inputs[0])
    node_group.links.new(group_inputs.outputs['Base Color_1'], mix_color.inputs['Color1'])
    node_group.links.new(group_inputs.outputs['Base Color_2'], mix_color.inputs['Color2'])
    node_group.links.new(mix_color.outputs[0], group_outputs.inputs['Base Color'])

    node_group.links.new(group_inputs.outputs['Normal_1'], group_outputs.inputs['Normal'])
    node_group.links.new(group_inputs.outputs['Specular_1'], group_outputs.inputs['Specular'])
    node_group.links.new(group_inputs.outputs['Roughness_1'], group_outputs.inputs['Roughness'])