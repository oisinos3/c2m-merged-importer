...
def createNodeGroup(codMaterial, images_path):
    ...
    #     node_multiply.inputs[1].default_value = 1.0
    ...
    for texture in codMaterial.Textures:
        ...
        if blender_image != None:
            ...
            node_image.label = texture.TexType
            node_image.name = texture.TexType

            # colorMap
            if texture.TexType == "colorMap":
                ...
                if texture.Name.endswith("_c"):
                    ...
                    links.new(node_image.outputs[1], group_outputs.inputs['Specular'])
                else:
                    node_group.outputs.new('NodeSocketFloat','Metallic')
                    links.new(node_image.outputs[1], group_outputs.inputs['Metallic'])
            ...
            # normalMap
                ...
                loc_y -= 300
            # specularMap
            # if "specularMap" in texture.TexType:
            #     node_image.image.colorspace_settings.name = "sRGB"
            #     node_image.location = (-250, loc_y)
            #     links.new(node_image.outputs[0], group_outputs.inputs['Base Color'])
            #     links.new(node_mapping.outputs[0], node_image.inputs[0])
            ...
            # mask
                ...
                links.new(sepRGB_node.outputs[1], group_outputs.inputs['Alpha'])
                links.new(node_mapping.outputs[0], node_image.inputs[0])
                loc_y -= 300
...
def createEmissiveMaterial(codMaterial):
    ...
    return material
def createMaterial(matList, map_materials):
    # Make material name
    ...
    principled_node.location = (650, -100)
    # Normal Map node
    ...
    if len(matList)>1:
        ...
        for index, mat in enumerate(matList):
            ...
            #     links.new(blend_nodes[index-1].outputs['Normal'], normal_node.inputs[1])

            location_y -= 200
        
        for output in blend_node.outputs:
            if output.name != "Alpha" and output.name != "Normal":
                links.new(blend_node.outputs[output.name], principled_node.inputs[output.name])
            elif output.name == "Normal":
                links.new(blend_node.outputs[output.name], normal_node.inputs[1])
    else:
        node_group = nodes.new('ShaderNodeGroup')
        ...
        for output in node_group.outputs:
            if output.name == "Normal":
                links.new(node_group.outputs[output.name], normal_node.inputs[1])
            else:
...
#     node_group.outputs.new('NodeSocketColor','Base Color')
#     node_group.outputs.new('NodeSocketColor','Normal')
#     node_group.outputs.new('NodeSocketColor','Metallic')
...
#     node_group.inputs['Normal_1'].default_value = [0.5, 0.5, 0.5, 1.0]
#     node_group.inputs.new('NodeSocketColor','Normal_2')
#     node_group.inputs['Normal_2'].default_value = [0.5, 0.5, 0.5, 1.0]
#     node_group.inputs.new('NodeSocketColor','Metallic_1')
#     node_group.inputs['Metallic_1'].default_value = [0.5, 0.5, 0.5, 1.0]
#     node_group.inputs.new('NodeSocketColor','Metallic_2')
#     node_group.inputs['Metallic_2'].default_value = [0.5, 0.5, 0.5, 1.0]
...
#     node_osl = node_group.nodes.new('ShaderNodeScript')
#     node_osl.mode = "EXTERNAL"
#     node_osl.filepath = "C:/work __ dev/OSL/test.osl"
#     node_osl.location = (-800, 0)
#     node_group.links.new(group_inputs.outputs['Vertex Alpha'], node_osl.inputs[0])
#     node_group.links.new(group_inputs.outputs['Alpha'], node_osl.inputs[4])
#     # Color MIX
...
#     node_group.links.new(node_osl.outputs[0], mix_color.inputs[0])
...
#     node_group.links.new(node_osl.outputs[0], mix_normal.inputs[0])
...
#     mix_specular.label = "MixMETALLIC"
#     node_group.links.new(group_inputs.outputs['Metallic_1'], mix_specular.inputs['Color1'])
#     node_group.links.new(group_inputs.outputs['Metallic_2'], mix_specular.inputs['Color2'])
#     node_group.links.new(node_osl.outputs[0], mix_specular.inputs[0])
#     node_group.links.new(mix_specular.outputs[0], group_outputs.inputs['Metallic'])
#     # Roughness MIX
...
#     node_group.links.new(node_osl.outputs[0], mix_roughness.inputs[0])
...
def createMixGroup():
    ...
    node_group.inputs['Alpha'].default_value = 1.0
    # Multiply node for alpha + vertex color
    node_multiply = node_group.nodes.new('ShaderNodeMath')
    node_multiply.operation = "MULTIPLY"
    node_multiply.location = (-800, 0)
    node_group.links.new(group_inputs.outputs['Vertex Alpha'], node_multiply.inputs[0])
    node_group.links.new(group_inputs.outputs['Alpha'], node_multiply.inputs[1])
    # Color MIX
    mix_color = node_group.nodes.new('ShaderNodeMixRGB')
    mix_color.location = (0, 0)
    mix_color.label = "MixCOLOR"
    node_group.links.new(group_inputs.outputs['Base Color_1'], mix_color.inputs['Color1'])
    node_group.links.new(group_inputs.outputs['Base Color_2'], mix_color.inputs['Color2'])
    node_group.links.new(node_multiply.outputs[0], mix_color.inputs[0])
    node_group.links.new(mix_color.outputs[0], group_outputs.inputs['Base Color'])
    # Normal MIX
    mix_normal = node_group.nodes.new('ShaderNodeMixRGB')
    mix_normal.location = (0, -200)
    ...
    node_group.links.new(group_inputs.outputs['Normal_1'], mix_normal.inputs['Color1'])
    node_group.links.new(group_inputs.outputs['Normal_2'], mix_normal.inputs['Color2'])
    node_group.links.new(node_multiply.outputs[0], mix_normal.inputs[0])
    node_group.links.new(mix_normal.outputs[0], group_outputs.inputs['Normal'])
    # Specular MIX
    mix_specular = node_group.nodes.new('ShaderNodeMixRGB')
    mix_specular.location = (0, -400)
    ...
    node_group.links.new(group_inputs.outputs['Metallic_1'], mix_specular.inputs['Color1'])
    node_group.links.new(group_inputs.outputs['Metallic_2'], mix_specular.inputs['Color2'])
    node_group.links.new(node_multiply.outputs[0], mix_specular.inputs[0])
    node_group.links.new(mix_specular.outputs[0], group_outputs.inputs['Metallic'])
    # Roughness MIX
    mix_roughness = node_group.nodes.new('ShaderNodeMixRGB')
    mix_roughness.location = (0, -600)
    ...
    node_group.links.new(group_inputs.outputs['Roughness_1'], mix_roughness.inputs['Color1'])
    node_group.links.new(group_inputs.outputs['Roughness_2'], mix_roughness.inputs['Color2'])
    node_group.links.new(node_multiply.outputs[0], mix_roughness.inputs[0])
    node_group.links.new(mix_roughness.outputs[0], group_outputs.inputs['Roughness'])