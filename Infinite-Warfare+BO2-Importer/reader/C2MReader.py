...
class CoDSurface(object):
    ...
    def __init__(self, file):
        ...
        for f in range(face_count):
            faceVerts = []
            # Read indices of triangle -- [0] position/normal/uv, [1] layerUV
            for x in range(3):
...
class CoDMesh(object):
    ...
    def __init__(self, file):
        ...
        for i in range(uv_count):
            # Read UV & add it
            uvSet_count = read_uint(file)
            uvSet = []
            for u in range(uvSet_count):
                uvSet.append([read_float(file),
                              read_float(file)])
            
            self.UVs.append(uvSet)
        # Read Colors
...
class CoDModelInstance(object):
    ...
    def __init__(self, file):
        ...
        # Read rotation
        self.Rotation_mode = read_byte(file)
        # Quaternion
        if self.Rotation_mode == 0:
            QuatRotation = [read_float(file), read_float(file), read_float(file), read_float(file)]
            self.Rotation = (QuatRotation[3], QuatRotation[0], QuatRotation[1], QuatRotation[2]) # Convert XYZW to WXYZ
        # Degrees
        elif self.Rotation_mode == 1:
            self.Rotation = (read_float(file), read_float(file), read_float(file))
            # Make euler variation
            self.Rotation_euler = (math.radians(self.Rotation[0]),
